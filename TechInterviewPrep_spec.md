# TechInterviewPrep — アプリケーション仕様書 v1.2
...
TechInterviewPrep は、北米のソフトウェアエンジニア職への転職を目指す開発者向けの技術面接対策Webアプリです。断片的な学習にとどまりがちな技術知識を、クイズ・口頭説明練習・進捗管理の3本柱で体系的に定着させることを目的としています。
...
| `/admin/generate` | 問題生成 | テキスト投入 or トピック指定 → AI分析 → 計画レビュー（編集可能）→ 問題生成 → プレビュー → 保存 |
...
> **注記**: explain タイプはクイズモードでは出題されず、専用の口頭説明モード (`/explain/[topicId]`) でのみ出題される。
...
| タイプ | 選択肢の構成 |
|---|---|
| 4択（multiple / code） | 4つの選択肢 + 「わからない」= **5択**で表示 |
| ○×（truefalse） | True / False + 「わからない」= **3択**で表示 |

- 「わからない」を選択した場合: `result = 'skipped'`、FSRS rating = `Again(1)`、解説は表示する

#### 「もう一度」ボタンの挙動

- 「もう一度」で再回答すると、progress レコードは最新の回答結果で上書き（upsert）される
...
1. ユーザーがテキスト/音声で回答
2. AIに送信: ユーザー回答 + ルーブリック（`options.rubric_ja` / `rubric_en`） + 模範解答（`answer`）
...
### 5-2. 問題生成フロー（AI自動判定型2ステップ方式）

#### 入力方式

| 方式 | 説明 |
|---|---|
| テキスト投入 | ドキュメント・記事・自分のメモなどをペーストする |
| トピック指定 | トピック名を選択する（テキストなしでAIが内容を判断） |
...
入力（テキスト貼付 or トピック選択）
       ...
       ↓ Step 1: AI分析（Claude API）

計画レビュー画面
  ├─ AI提案の問題構成を表示（タイプ・難易度・問題数・理由）
  ├─ 面接関連度（high / medium / low）とコンテンツ要約を表示
  ├─ 管理者が問題数の増減、タイプの追加削除、難易度変更が可能
  └─ 「生成」ボタン

       ↓ Step 2: 問題生成（Claude API）
...
| ルール | 詳細 |
|---|---|
| multiple必須 | 常に含める、全体の40-60% |
| codeゲート | プログラミング構文が自然なコンテンツのみ。純理論トピック（CSS設計哲学、アクセシビリティ原則等）では0 |
| truefalse上限 | 最大4問 |
| explain最低1 | interview_relevance="high"なら最低1問 |
| explain上限 | 最大3問 |
| 総数範囲 | 5-15問 |
| 難易度デフォルト | easy 30% / medium 50% / hard 20% |
...
### 7-1. Step 1: コンテンツ分析（Analysis）— Claude API
...
```json
{
  "topic_summary": "コンテンツの要約",
  "interview_relevance": "high | medium | low",
  "interview_relevance_reason": "面接関連度の理由",
  "questions": [
    {
...
### 7-2. Step 2: 問題生成（Generation）— Claude API
  ...
  "questions": [
    {
      ...
      "question_ja": "問題文（日本語）",
      "question_en": "問題文（英語）",
      "options": "選択肢配列 | ルーブリックオブジェクト | null",
      "answer": "正解データ | 模範解答",
      "explanation_ja": "解説（日本語）",
      "explanation_en": "解説（英語）"
    }
  ]
}
```
...
> `{rubric}` は DB の `questions.options.rubric_ja` / `rubric_en`（ユーザー言語に応じて選択）、`{modelAnswer}` は `questions.answer.model_answer_ja` / `model_answer_en` から取得します。
...
| クロージャ・スコープ | Medium | ★★★ |
| プロトタイプ・継承 | Medium | ★★★ |
| 非同期処理（Promise / async-await / イベントループ） | Hard | ★★★ |
| this キーワード | Medium | ★★★ |
