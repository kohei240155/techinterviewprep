...
| AI口頭説明フィードバック・音声認識 | OpenAI API (GPT + Whisper) | 口頭説明フィードバック・音声入力 |
...
面接本番を想定した口頭説明の練習モードです。`type = 'explain'` の問題が DB に事前登録されており、採点基準（`options`）と模範解答（`answer`）を持ちます。ユーザーがテキスト入力または音声入力（Whisper）で回答すると、AIが採点基準に基づいて面接官視点のフィードバックを返します。
...
フィードバックはOpenAI API (GPT) により、ユーザー設定の言語で返されます。DB に保存された採点基準（正確性・網羅性・明瞭さ等）に基づきスコアリングします。
...
| options | jsonb | 選択肢（multiple/code）or 採点基準（explain） |
...
### 7-3. 口頭説明フィードバック（OpenAI GPT）

> `{language}` はユーザー設定の言語（`ja` or `en`）に動的に切り替わります。
> `{criteria}` は DB の `questions.options`（採点基準配列）、`{modelAnswer}` は `questions.answer`（模範解答）から取得します。

```
System:
  あなたは北米テック企業の面接官です。
  回答言語: {language}
  以下の採点基準に基づいて、候補者の回答を {language} で評価してください。
  採点基準: {criteria}
  模範解答: {modelAnswer}
  フィードバック内容:
  1. 採点基準ごとのスコア（0-100）とコメント
  2. 説明の不足点・誤り
  3. より自然な英語表現の提案
