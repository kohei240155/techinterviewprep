...
面接本番を想定した口頭説明の練習モードです。選択したトピックに対して、面接官役のプロンプトが表示されます。ユーザーがテキスト入力または音声入力（Whisper）で回答すると、AIが面接官視点でフィードバックを返します。
...
フィードバックはOpenAI API (GPT) により、ユーザー設定の言語で返されます。
