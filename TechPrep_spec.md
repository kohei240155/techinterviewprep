...
| AI生成・採点 | Anthropic API (Claude) | 問題生成・フィードバック・重複チェックを統一 |
...
| `/settings` | ユーザー設定 | 言語設定（ja / en）の切り替え |
| `/topics/[topicId]` | トピック詳細 | 問題一覧・習熟度・苦手フラグ |
...
問題を1問ずつカード形式で表示します。ユーザー設定の言語（ja / en）で問題文・解説を表示します。回答後に解説を展開表示し、理解を深めます。10問ごとに進捗サマリーを挿入します。ログインなしでも利用可能です（ゲスト利用時は進捗が保存されません）。
...
- 解説：ユーザー設定の言語で表示（設定画面で切り替え可能）
- 自己評価（ログインユーザーのみ）：回答後に Again / Hard / Good / Easy の4段階で自己評価。FSRS アルゴリズムにより次回復習日を自動算出
- アクション：「次の問題へ」「もう一度」「苦手登録」の3ボタン

---

### 4-2. 口頭説明モード

面接本番を想定した口頭説明の練習モードです。選択したトピックに対して、面接官役のプロンプトが表示されます。ユーザーがテキストで回答を入力すると、AIが面接官視点でフィードバックを返します。

#### フィードバック内容

フィードバックはユーザー設定の言語で返されます。
...
- ログインすると進捗管理・復習スケジューリング・設定の永続化が有効になる
...
| language | text | ja / en（デフォルト: ja） |
| updated_at | timestamp | 更新日時 |

> **未ログインユーザーの言語設定**: localStorage に `{ language: "ja" }` として保存。ログイン時に DB へマージする。
...
|---|---|---|
| id | uuid (PK) | 主キー |
| name | text | トピック名（日本語）例：非同期処理 |
| name_en | text | トピック名（英語）例：Async / Promises |
| category | text | カテゴリ例：JavaScript / TypeScript |
| icon | text | 絵文字アイコン例：⚡ |
...
| type | text | multiple / code / truefalse |
| difficulty | text | easy / medium / hard |
| question_ja | text | 問題文（日本語） |
| question_en | text | 問題文（英語） |
| options | jsonb | 4択の選択肢配列（4択のみ） |
| answer | jsonb | 正解（index または boolean） |
| explanation_ja | text | 解説（日本語） |
| explanation_en | text | 解説（英語） |
| source_text | text | 生成元テキスト（任意） |
| created_at | timestamp | 作成日時 |
...
System:
  あなたは北米テック企業の面接官です。
  回答言語: {language}
  候補者の回答に対して、以下の観点で {language} でフィードバックしてください。
  1. 説明の不足点・誤り
  2. より自然な英語表現の提案
  3. 模範解答
