...
| is_bookmarked | `boolean` | NOT NULL | `false` | ブックマークフラグ |
...
- `idx_progress_bookmarks` — `(user_id)` WHERE `is_bookmarked = true`（ブックマーク一覧用、部分インデックス）
...
CREATE INDEX idx_progress_bookmarks
  ON public.progress (user_id)
...
| **部分インデックス** | `due_date IS NOT NULL` / `is_bookmarked = true` の部分インデックスで、復習対象・ブックマーク一覧クエリのパフォーマンスを最適化 |
...
| **CASCADE 削除** | `users` 削除時に `progress` も連鎖削除。`categories` 削除時に `topics`、`topics` 削除時に `questions` も連鎖削除。データ孤立を防止 |
