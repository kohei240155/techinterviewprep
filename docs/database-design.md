...
erDiagram
    ...
    progress {
        ...
        int rating "クイズ:正解=3,不正解=1 / 説明:AI評価1-4"
        int reps
        int lapses
        int state "0-3"
...
| display_name | `text` | NOT NULL | `''` | Google アカウント表示名 |
| avatar_url | `text` | NOT NULL | `''` | Google アバター URL |
...
| options | `jsonb` | — | `NULL` | 選択肢（multiple/code）or ルーブリック（explain） |
...
| rating | `int` | CHECK (`rating BETWEEN 1 AND 4`) | `NULL` | クイズ: 自動マッピング（正解=3, 不正解=1）/ 説明: AIが1-4で評価 |
...
`type = 'multiple'` / `'code'` の場合は選択肢配列、`type = 'explain'` の場合はルーブリック（理解度の4段階基準）オブジェクトを格納する。`type = 'truefalse'` の場合は `NULL`。
...
// explain の例（ルーブリック — 理解度の4段階基準）
{
  "rubric_ja": {
    "1": "thisの基本的な仕組みを説明できていない",
    "2": "thisがコンテキストに依存することは理解しているが、具体的なルールを説明できない",
    "3": "暗黙的・明示的バインディング、アロー関数での挙動を正しく説明できる",
    "4": "全バインディングルールの優先順位やエッジケースまで深く説明できる"
  },
  "rubric_en": {
    "1": "Cannot explain the basic mechanism of 'this'",
    "2": "Understands that 'this' depends on context, but cannot explain specific rules",
    "3": "Can correctly explain implicit/explicit binding and arrow function behavior",
    "4": "Can deeply explain priority of all binding rules and edge cases"
  }
}
```
...
```sql
-- ============================================================
-- TechInterviewPrep 初期スキーマ マイグレーション
-- ============================================================
...
INSERT INTO public.categories (id, name_ja, name_en, sort_order) VALUES
  ...
  ('a1b2c3d4-0001-4000-8000-000000000002', 'React / フロントエンド',  'React / Frontend',       2);
...
| **`options` が nullable** | `truefalse` 問題には選択肢が不要なため `NULL` を許容。`multiple` / `code` では選択肢配列、`explain` ではルーブリックオブジェクトを格納 |
| **`explain` タイプ** | 口頭説明モード用の問題を questions テーブルに統合。`options` にルーブリック（理解度の4段階基準）、`answer` に模範解答を格納。AIがルーブリックに基づいて1-4で評価し、そのまま FSRS rating として使用。別テーブルにせず type で区別することでクエリとスキーマをシンプルに保つ |
